from .gamlss_ext import BCPEModel, BetaModel, GAMLSSModelByCondition
from .gamlss_params import FractionalPolynomial, LogitLink, LogLink, PSpline

__all__ = [
    "csfFF",
    "csfMM",
    "hcff",
    "hcmm",
    "hcxx",
    "tissFF",
    "tissMM",
    # "_ratioF",
    # "_ratioM",
    # "tcF",
    # "tcM",
    # "ciratioF",
    # "ciratioM",
    # "ciratioF_beta",
    # "ciratioM_beta",
]


hcflh_female = BCPEModel(
    mu=FractionalPolynomial(
        intercept=-274.7848237296913,
        coefficients=(-149.84508855999425, -48.00727849400349, 324.87772798513777),
        fpowers=(0.0, 0.0, 0.5),
        domain=(48.5, 180.5),
        shift=0,
    ),
    sigma=LogLink(-3.5834762072532387),
    nu=-0.5756035007094116,
    tau=LogLink(0.5563002460403006),
    attrs={"long_name": "Head Circumference", "units": "cm"},
    x_attrs={"name": "len_hi", "long_name": "Recumbent Length / Standing Height", "units": "cm"},
)
# DEPRECATED
hcff = hcflh_female
hcflh_male = BCPEModel(
    mu=FractionalPolynomial(
        intercept=692.5722663851999,
        coefficients=(-641.4702714565977, -309.7469221642888, 72.44232181418973),
        fpowers=(-0.5, 0.0, 0.0),
        domain=(51, 195.6),
        shift=0,
    ),
    sigma=LogLink(-3.6385438880994867),
    nu=-0.020559517437386845,
    tau=LogLink(0.4495271825950411),
    attrs={"long_name": "Head Circumference", "units": "cm"},
    x_attrs={"name": "len_hi", "long_name": "Recumbent Length / Standing Height", "units": "cm"},
)
# DEPRECATED
hcmm = hcflh_male
hcflh = GAMLSSModelByCondition(hcflh_female, hcflh_male, cond_attrs={"name": "is_female", "long_name": "sex = Female"})
hcflh_xx = BCPEModel(
    mu=FractionalPolynomial(
        intercept=-422.2940844051155,
        coefficients=(126.44541423086386, -398.12576972488137, 346.5552682948275),
        fpowers=(0.5, 0.5, 1.0),
        domain=(48.5, 195.6),
        shift=0,
    ),
    sigma=LogLink(-3.5738656336728867),
    nu=-0.28912191253351954,
    tau=LogLink(0.5328533922094477),
    attrs={"long_name": "Head Circumference", "units": "cm"},
    x_attrs={"name": "len_hi", "long_name": "Recumbent Length / Standing Height", "units": "cm"},
)
# DEPRECATED
hcxx = hcflh_xx
cvfa_female = BCPEModel(
    mu=FractionalPolynomial(
        intercept=174.36608687335067,
        coefficients=(0.4198602329356907, -17.850485999978638, 0.10215264932793458),
        fpowers=(-1.0, -0.5, 3.0),
        domain=(-21, 8131),
        shift=22,
    ),
    sigma=LogLink(-1.615757113209806),
    nu=0.42546112691657323,
    tau=LogLink(0.17507009002314125),
    attrs={"long_name": "CSF Volume", "units": "cm^3"},
    x_attrs={"name": "age", "long_name": "Age", "units": "days"},
)
# DEPRECATED
csfFF = cvfa_female
cvfa_male = BCPEModel(
    mu=FractionalPolynomial(
        intercept=190.4495277278507,
        coefficients=(0.44937314250337496, -19.16707542424443, 0.10453302112506725),
        fpowers=(-1.0, -0.5, 3.0),
        domain=(-21, 8073),
        shift=22,
    ),
    sigma=LogLink(-1.5987646631876429),
    nu=0.6562282247254976,
    tau=LogLink(0.17380717231195147),
    attrs={"long_name": "CSF Volume", "units": "cm^3"},
    x_attrs={"name": "age", "long_name": "Age", "units": "days"},
)
# DEPRECATED
csfMM = cvfa_male
cvfa = GAMLSSModelByCondition(cvfa_female, cvfa_male, cond_attrs={"name": "is_female", "long_name": "sex = Female"})
bvfa_female = BCPEModel(
    mu=FractionalPolynomial(
        intercept=-32.40188886990359,
        coefficients=(6.8243795721453555, 1029.4548916362535, -302.4064745323913),
        fpowers=(-0.5, 0.5, 0.5),
        domain=(-21, 8131),
        shift=22,
    ),
    sigma=LogLink(-2.5310152523814264),
    nu=-0.4708322009246219,
    tau=LogLink(0.7144038214357605),
    attrs={"long_name": "Brain Tissue", "units": "cm^3"},
    x_attrs={"name": "age", "long_name": "Age", "units": "days"},
)
# DEPRECATED
tissFF = bvfa_female
bvfa_male = BCPEModel(
    mu=FractionalPolynomial(
        intercept=-85.36920326404015,
        coefficients=(7.81838294854985, 1187.17576187236, -348.1199770487963),
        fpowers=(-0.5, 0.5, 0.5),
        domain=(-21, 8073),
        shift=22,
    ),
    sigma=LogLink(-2.6171047897119526),
    nu=0.6989205425456295,
    tau=LogLink(0.40047791511800596),
    attrs={"long_name": "Brain Tissue", "units": "cm^3"},
    x_attrs={"name": "age", "long_name": "Age", "units": "days"},
)
# DEPRECATED
tissMM = bvfa_male
bvfa = GAMLSSModelByCondition(bvfa_female, bvfa_male, cond_attrs={"name": "is_female", "long_name": "sex = Female"})
## FIXME: broken upstream
_bv_cv_ratio_for_age_female = BCPEModel(
    mu=FractionalPolynomial(
        intercept=11.082874194353732,
        coefficients=(-4.805927794274573, 3.3948503231064917, -1.021721471224189),
        fpowers=(0.5, 0.5, 0.5),
        domain=(-21, 8131),
        shift=22,
    ),
    sigma=LogLink(-1.6170309696939928),
    nu=0.05701029564202015,
    tau=LogLink(0.3832294345648013),
    attrs={"long_name": "Brain Tissue / CSF Volume Ratio"},
    x_attrs={"name": "age", "long_name": "Age", "units": "days"},
)
# DEPRECATED
_ratioF = _bv_cv_ratio_for_age_female
## FIXME: broken upstream
_bv_cv_ratio_for_age_male = BCPEModel(
    mu=FractionalPolynomial(
        intercept=-10.811516509095394,
        coefficients=(-2.580157458459326, 17.191469945479266, -4.655185756173699),
        fpowers=(0.0, 0.5, 0.5),
        domain=(-21, 8073),
        shift=22,
    ),
    sigma=LogLink(-1.6422083676281767),
    nu=-0.7371007188630626,
    tau=LogLink(0.10333210663154822),
    attrs={"long_name": "Brain Tissue / CSF Volume Ratio"},
    x_attrs={"name": "age", "long_name": "Age", "units": "days"},
)
# DEPRECATED
_ratioM = _bv_cv_ratio_for_age_male
_bv_cv_ratio_for_age = GAMLSSModelByCondition(
    _bv_cv_ratio_for_age_female,
    _bv_cv_ratio_for_age_male,
    cond_attrs={"name": "is_female", "long_name": "sex = Female"},
)
bv_cv_ratio_for_age_female = BCPEModel(
    mu=FractionalPolynomial(
        intercept=7.451604029302721,
        coefficients=(1.9311003202293868, 0.2614952054465591, -0.9333994900219552),
        fpowers=(0.0, 0.0, 1.0),
        domain=(14, 8131),
        shift=0,
    ),
    sigma=LogLink(-1.7177909182272781),
    nu=0.23164241283324605,
    tau=LogLink(0.45456869049681214),
    attrs={"long_name": "Brain Tissue / CSF Volume Ratio"},
    x_attrs={"name": "age", "long_name": "Age", "units": "days"},
)
# DEPRECATED
tcF = bv_cv_ratio_for_age_female
bv_cv_ratio_for_age_male = BCPEModel(
    mu=FractionalPolynomial(
        intercept=7.538670088628258,
        coefficients=(2.2650666623903875, 0.3319242947160484, -1.0650611097770506),
        fpowers=(0.0, 0.0, 1.0),
        domain=(13, 8073),
        shift=0,
    ),
    sigma=LogLink(-1.7492290015068284),
    nu=0.019907631818795388,
    tau=LogLink(0.29597479412370803),
    attrs={"long_name": "Brain Tissue / CSF Volume Ratio"},
    x_attrs={"name": "age", "long_name": "Age", "units": "days"},
)
# DEPRECATED
tcM = bv_cv_ratio_for_age_male
bv_cv_ratio_for_age = GAMLSSModelByCondition(
    bv_cv_ratio_for_age_female, bv_cv_ratio_for_age_male, cond_attrs={"name": "is_female", "long_name": "sex = Female"}
)
cv_icv_prop_for_age_female = BCPEModel(
    mu=PSpline(
        domain=(14, 8131),
        intercept=0.1297789277794307,
        slope=6.923203390882593e-07,
        spline_coefficients=(
            0.09008907611394162,
            0.061995418771109916,
            0.03459952684997254,
            0.012931811764261322,
            -3.957351711353667e-05,
            -0.006111783656124427,
            -0.008890247628657961,
            -0.009833284615727078,
            -0.008846850697730663,
            -0.006549756391621697,
            -0.006205336855025153,
            -0.0061558214599097705,
            -0.004231271312435878,
            -0.0020108634738736455,
            0.0009661845033298209,
            0.0035492814045548662,
            0.007218651799795825,
            0.01129005734515242,
            0.015224830859958207,
            0.01891205965701592,
            0.023465764448297965,
            0.028825171788305116,
            0.03427722496953207,
        ),
        spline_degree=3,
    ),
    sigma=LogLink(-1.8580057025189702),
    nu=-0.05880935174072106,
    tau=LogLink(0.49534771358580737),
    attrs={"long_name": "CSF Volume / Intracranial Volume Proportion"},
    x_attrs={"name": "age", "long_name": "Age", "units": "days"},
)
# DEPRECATED
ciratioF = cv_icv_prop_for_age_female
cv_icv_prop_for_age_male = BCPEModel(
    mu=PSpline(
        domain=(13, 8073),
        intercept=0.13444173398569476,
        slope=-7.331276817827039e-07,
        spline_coefficients=(
            0.11588494438632609,
            0.0728158791534432,
            0.031735211954082486,
            0.007414429680544646,
            -0.00018644841367380287,
            -0.004518353895951099,
            -0.010053197526453693,
            -0.013771096948748583,
            -0.013678810549509268,
            -0.011530812390913798,
            -0.007704730171405147,
            -0.005644932139213118,
            -0.004522606739621292,
            -0.004768874577389469,
            -0.0020083977727083074,
            0.004378502164301235,
            0.011085664337895254,
            0.015905647935543946,
            0.019184065678930617,
            0.02171518032443226,
            0.023776083958618115,
            0.024953851354403058,
            0.026009552583848003,
        ),
        spline_degree=3,
    ),
    sigma=LogLink(-1.8971241624545456),
    nu=0.15514658132713416,
    tau=LogLink(0.3371065157124078),
    attrs={"long_name": "CSF Volume / Intracranial Volume Proportion"},
    x_attrs={"name": "age", "long_name": "Age", "units": "days"},
)
# DEPRECATED
ciratioM = cv_icv_prop_for_age_male
cv_icv_prop_for_age = GAMLSSModelByCondition(
    cv_icv_prop_for_age_female, cv_icv_prop_for_age_male, cond_attrs={"name": "is_female", "long_name": "sex = Female"}
)
cv_icv_prop_for_age_beta_female = BetaModel(
    mu=LogitLink(
        PSpline(
            domain=(14, 8131),
            intercept=-1.8634996247920739,
            slope=2.34740332507667e-06,
            spline_coefficients=(
                0.5756341624377102,
                0.40975972089855595,
                0.2468031912704036,
                0.10796362388882355,
                0.007524764346361232,
                -0.05546930921679457,
                -0.09025499286155975,
                -0.10215776973173293,
                -0.093482050564027,
                -0.07294013325118749,
                -0.06075134232595502,
                -0.0535264928919104,
                -0.03937034056565001,
                -0.020805307284029517,
                0.0034621104834925438,
                0.026890772594026762,
                0.05564464229222489,
                0.08674896670793984,
                0.1171371760982431,
                0.14679280775809683,
                0.18263603388603952,
                0.22350045367085816,
                0.26495358877597097,
            ),
            spline_degree=3,
        )
    ),
    sigma=LogitLink(
        PSpline(
            domain=(14, 8131),
            intercept=-2.527426313583957,
            slope=-5.235696325653294e-05,
            spline_coefficients=(
                0.1728590329837209,
                0.13774700140318735,
                0.10259114834341108,
                0.0680794008925337,
                0.03623207042844137,
                0.008606079352525354,
                -0.01412426493123175,
                -0.03097165627987182,
                -0.04100754609631964,
                -0.04430733714641174,
                -0.041483865767280956,
                -0.03353322091811403,
                -0.021622595401803172,
                -0.007527759866486923,
                0.007076974047702424,
                0.020823150249615608,
                0.03351908480896952,
                0.04541152005761799,
                0.05656018257787499,
                0.06693971111728514,
                0.07648831476352487,
                0.08581458508573714,
                0.0951347526164553,
            ),
            spline_degree=3,
        )
    ),
    attrs={"long_name": "CSF Volume / Intracranial Volume Proportion"},
    x_attrs={"name": "age", "long_name": "Age", "units": "days"},
)
# DEPRECATED
ciratioF_beta = cv_icv_prop_for_age_beta_female
cv_icv_prop_for_age_beta_male = BetaModel(
    mu=LogitLink(
        PSpline(
            domain=(13, 8073),
            intercept=-1.8284523225480405,
            slope=-1.0058190414091444e-05,
            spline_coefficients=(
                0.6751169833278396,
                0.4424823390151559,
                0.21873654032303413,
                0.0646381528383861,
                -0.013856029537431263,
                -0.05891519008821124,
                -0.09738567478721781,
                -0.1264142946580269,
                -0.13357423807989796,
                -0.11904129557683639,
                -0.08688017295397077,
                -0.06166782814053265,
                -0.05352906871768858,
                -0.05481403178368132,
                -0.03438641120676677,
                0.018075956181226118,
                0.08380421646701765,
                0.13947258871811638,
                0.17851809594234744,
                0.20245657216082702,
                0.21533643985551615,
                0.2229524789172864,
                0.2303745810921812,
            ),
            spline_degree=3,
        )
    ),
    sigma=LogitLink(
        PSpline(
            domain=(13, 8073),
            intercept=-2.6438444716043463,
            slope=-3.372026660480111e-05,
            spline_coefficients=(
                0.24267941491630793,
                0.1909739027751034,
                0.13922683979294803,
                0.08894342827437064,
                0.045013958038757386,
                0.009491057808366267,
                -0.01852171797546217,
                -0.04042825389337072,
                -0.056457474420113035,
                -0.0677193203099513,
                -0.07539905237190936,
                -0.078631324209598,
                -0.07480551109369322,
                -0.061121012295806416,
                -0.036173016262491,
                -0.0004602812762576,
                0.043895308438883576,
                0.09424173922052563,
                0.14777493882824158,
                0.20282194121659539,
                0.2583678957894611,
                0.31376298641115574,
                0.3691272072933926,
            ),
            spline_degree=3,
        )
    ),
    attrs={"long_name": "CSF Volume / Intracranial Volume Proportion"},
    x_attrs={"name": "age", "long_name": "Age", "units": "days"},
)
# DEPRECATED
ciratioM_beta = cv_icv_prop_for_age_beta_male
cv_icv_prop_for_age_beta = GAMLSSModelByCondition(
    cv_icv_prop_for_age_beta_female,
    cv_icv_prop_for_age_beta_male,
    cond_attrs={"name": "is_female", "long_name": "sex = Female"},
)
